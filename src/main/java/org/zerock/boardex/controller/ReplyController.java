...
public class ReplyController {
    ...
    // 댓글 목록 처리
    @Operation(summary = "게시글의 댓글 목록", description = "GET 방식으로 특정 게시물의 댓글 목록")
    @GetMapping(value = "/list/{bno}")
    // @PathVariable("bno") : ?bno=197 와 같은 쿼리스트링의 값을 전달받음
    // 페이징 객체
    ...
    @GetMapping("/{rno}")
    // PathVariable("rno") : ?rno=10 와 같은 쿼리스트링의 값을 전달받음
    ...
    // consumes = MediaType.APPLICATION_JSON_VALUE : 클라이언트가 서버에게 보내는 데이터 타입을 명시함(JSON 형식)
    @PutMapping(value = "/{rno}", consumes = MediaType.APPLICATION_JSON_VALUE)
    // @PathVariable("rno") : ?rno=10 과 같은 쿼리스트링 만듬
    // @RequestBody ReplyDTO replyDTO : xml이나 json기반의 메시지를 요청할 때 사용
