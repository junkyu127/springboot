...
public class BoardController {
    ...
    @PreAuthorize("hasRole('USER')") // 특정 사용자만 권한 설정
    ...
    public String modify(PageRequestDTO pageRequestDTO, @Valid BoardDTO boardDTO, BindingResult bindingResult, RedirectAttributes redirectAttributes){
        ...
        if(bindingResult.hasErrors()) {
            // 문자열 기록
            log.info("에러 있어요..");
            // pageRequestDTO 객체의 getLink() 메서드 호출한 결과를 link에 저장
            // link="page=3&size=10&type=tcw&keyword=자바"
            String link=pageRequestDTO.getLink();
            // bindingResult 객체의 모든 에러를 "errors" 라는 이름으로 POST방식으로 서버에 전송 -> [[${errors}]]
            redirectAttributes.addFlashAttribute("errors", bindingResult.getAllErrors());
            // 데이터의 번호(bno)를 bno 라는 이름으로 서버에 POST 방식으로 전송
            redirectAttributes.addAttribute("bno", boardDTO.getBno());
            // 쿼리스트링 : /board/modify?page=3&size=10&type=tcw&keyword=자바 -> 이 주소로 이동함
            return "redirect:/board/modify?"+link;
        }
        // boardSerivce 의 modify 함수 호출하면서 boardDTO 객체 넘김
        boardService.modify(boardDTO);
        // 데이터 이름은 result, 값은 modified로 서버에 POST 방식으로 데이터 전달
        redirectAttributes.addFlashAttribute("result","modified");
        // 수정된 데이터의 번호(bno)를 bno 라는 이름으로 서버에 전달
        redirectAttributes.addAttribute("bno", boardDTO.getBno());
        // board/read 로 이동
    ...
    public String remove(BoardDTO boardDTO, RedirectAttributes redirectAttributes){
        ...
        // boardService 객체의 remove 메서드 호출하면서 bno(번호) 를 넘김
        ...
        // /board/list 로 이동, 정상 사용자가 아니면 로그인 화면으로 이동
        return "redirect:/board/list";
    }
