...
public interface BoardService {
    ...
    // 디폴트 메서드(defalut method)를 구현하여 그대로 자식 객체로 상속해 줄 수 있음.
    default Board dtoToEntity(BoardDTO boardDTO) {
        ...
        if(boardDTO.getFileNames() != null){
            ...
            boardDTO.getFileNames().forEach(fileName -> {
                ...
                // arr[0] = 15f0b0cb-484a-4d68-936f-4055712d2bc8
                // arr[1] = flower.jpg
    ...
    default BoardDTO entityToDTO(Board board) {
        BoardDTO boardDTO = BoardDTO.builder()
                ...
                .build();
        // 첨부파일의 Uuid + "_" + 파일명 합친 문자열을 fileNames 에 배열로 저장
        List<String> fileNames = board.getImageSet().stream().sorted().map(boardImage ->
            boardImage.getUuid() + "_" + boardImage.getFileName()).collect(Collectors.toList());
        // Uuid + "_" + 파일명으로 만들어진 fileNames 의 값을 boardDTO 객체의 fileName 에 저장
