...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        // OAuth2User : 로그인 정보(타입 객체)
        OAuth2User oAuth2User = super.loadUser(userRequest);
        // OAuth2User 에서 얻어온 속성 정보를 Map 배열 형태로 paramMap 에 저장
...
//        });
    ...
    // 카카오 서비스에서 얻어온 이메일을 이용해 같은 이메일을 가진 사용자가 없으면 자동으로 회원가입하고
    // MemberSecurityDTO 를 반환함.
    private MemberSecurityDTO generatedDTO(String email, Map<String, Object> params) {
        // 데이터베이스에서 이메일을 가져와서 result 에 저장
        Optional<Member> result = memberRepository.findByEmail(email);

        // 데이터베이스에 해당 이메일을 사용하는 사용자가 없다면
        if(result.isEmpty()) {
            // 회원 추가, 여기서 mid = email, 비밀번호 = 1111
            ...
            // return 값이 비어있지 않다면(이미 회원이라면)
            return memberSecurityDTO;
        } else {
            // 회원정보를 가져와서 memberSecurityDTO 에 저장
    ...
    private String getKakaoEmail(Map<String, Object> paramMap) {
        ...
        log.info(value);
        // LinkHashMap : Map 에 입력된 순서를 기억하는 자료구조임.
