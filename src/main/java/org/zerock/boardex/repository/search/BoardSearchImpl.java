...
public class BoardSearchImpl extends QuerydslRepositorySupport implements BoardSearch {
    ...
    public Page<BoardListReplyCountDTO> searchWithReplyCount (String[] types, String keyword, Pageable pageable) {
        // Board 의 속성을 이용하여 다양한 SQL 구문을 처리할 수 있는 객체임.
        QBoard board = QBoard.board;
        QReply reply = QReply.reply;
        // 일반 쿼리(Sql) 은 data 입장에서 실행
        ...
        JPQLQuery<BoardListReplyCountDTO> dtoQuery = query.select(
                ...
                // 조회할 때 어떤 것을 기반으로 조회할 것인지를 설정하는 메서드임.
        ...
        // dtoQuery(게시글에서 번호, 제목, 내용, 작성자, 댓글개수를 조회한 결과)의 내용을 페이징하기 쉽게 applyPagination 메서드 사용.
        this.getQuerydsl().applyPagination(pageable, dtoQuery);
        // dtoQuery(조회결과) 를 배열 객체로 dtoList 에 저장함.
    ...
    public Page<BoardListAllDTO> searchWithAll(String[] types, String keyword, Pageable pageable) {
        // Querydsl 이용한 동적 SQL 의 처리
        QBoard board = QBoard.board;
        QReply reply = QReply.reply;
        // 게시판 객체에 자바 친화적인 질의어를 실행할 수 있도록 설정
        JPQLQuery<Board> boardJPQLQuery = from(board);
        // reply 객체랑 board 객체를 left outer join
        boardJPQLQuery.leftJoin(reply).on(reply.board.eq(board));
        // 검색 조건과 키워드 사용
        // types가 null이 아니고 길이가 0보다 크고, keyword 값이 null이 아니라면
        if((types != null && types.length > 0) && keyword != null) {
        ...
        // countDistinct() : 중복 제거한 값
        JPQLQuery<Tuple> tupleJPQLQuery = boardJPQLQuery.select(board, reply.countDistinct());
        // left outer join한 결과 + paging 결과 값을 boardList 배열로 저장
